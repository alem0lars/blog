    ...
    def run(self):
        ...
        try:
            ...
            error = self.state_machine.reporter.error(
                'Failed to run plantuml: {}'.format(exc),
            ...
            if p.returncode == 0:
                ...
                filename = "%08x" % (adler32(body.encode('utf8')) & 0xffffffff)
                newname = os.path.join(path, filename) + imgext
...
def custom_url(generator, metadata):
    ...
    if 'PLANTUML_PREAMBLE' in generator.settings:
        preamble = generator.settings['PLANTUML_PREAMBLE']
